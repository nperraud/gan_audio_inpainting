...
class InpaintingGAN(SpectrogramGAN):
    def default_params(self):
        d_params = deepcopy(super().default_params())
    ...
    def _build_net(self):
        ...
        print("consistency_contribution", consistency_contribution)
        self._R_Con = self.consistency((self.X_real-1)*5)
        self._F_Con = self.consistency((self.X_fake_center-1)*5)
        self._mean_R_Con, self._std_R_Con = tf.nn.moments(self._R_Con, axes=[0])
        self._mean_F_Con, self._std_F_Con = tf.nn.moments(self._F_Con, axes=[0])
    ...
    def _build_generator(self): #self.params['optimization']['batch_size']
        shape = self._params['shape']
        self.X_real = tf.placeholder(tf.float32, shape=[None, *shape], name='Xreal')
        self.z = tf.placeholder(
            tf.float32,
        ...
        borderleft, self.center_real, borderright = tf.split(self.X_to_inpaint, self.params['inpainting']['split'], axis=2)
...
class MultipleDiscrimnatorInpaintingGAN(InpaintingGAN):
    def _build_net(self):
        self._data_size = self.params['generator']['data_size']
        assert(self.params['discriminator']['data_size'] == self.data_size)
        ...
        for scale in [1, 2, 4]:
            time = self.X_fake.shape[2]
            start = int((384-96*scale)/2)
            end = 384 - start
            x_fake = self.X_fake[:,:,start:end:scale]
            x_real = self.X_real[:,:,start:end:scale]
        
            discriminator_name = "discriminator"
            if scale != 4:
                discriminator_name += str(scale)
        
            self._D_fake = self.discriminator(x_fake, reuse=False, scope=discriminator_name)
            self._D_real = self.discriminator(x_real, reuse=True, scope=discriminator_name)
            self._D_loss_f += tf.reduce_mean(self._D_fake)
            self._D_loss_r += tf.reduce_mean(self._D_real)
            
            gamma_gp = self.params['gamma_gp']
            self._D_gp += self.wgan_regularization(gamma_gp, [x_fake], [x_real], discriminator_name)
    ...
    def wgan_regularization(self, gamma, list_fake, list_real, discriminator_name):
        if not gamma:
            # I am not sure this part or the code is still useful
            t_vars = tf.trainable_variables()
            d_vars = [var for var in t_vars if 'discriminator' in var.name]
            ...
            D_x_hat = self.discriminator(*x_hat, reuse=True, scope=discriminator_name)
