...
class GANsystem(NNSystem):
    ...
    def train(self, dataset, **kwargs):
        if self.params['Nstats']:
            ...
            self.summary_dataset = dataset.batch(batch_size=int(self.params['Nstats']/self.params['optimization']['batch_size']))
            self.summary_dataset = self.summary_dataset.map(map_func=lambda x: tf.reshape(x, [int(self.params['Nstats']/self.params['optimization']['batch_size'])*self.params['optimization']['batch_size'], self.params['net']['shape'][0], self.params['net']['shape'][1]]), num_parallel_calls=tf.data.experimental.AUTOTUNE)
            self.summary_dataset = tf.compat.v1.data.make_one_shot_iterator(self.summary_dataset).get_next()
            self.need_to_preprocess_summaries = True
    ...
    def _train_log(self, feed_dict):
        ...
        if self.params['Nstats']:
            if self.need_to_preprocess_summaries:
                self.net.preprocess_summaries(self._sess.run(self.summary_dataset), rerun=False)
                self.need_to_preprocess_summaries = False

            sum_batch = self._sess.run(self.summary_dataset)
